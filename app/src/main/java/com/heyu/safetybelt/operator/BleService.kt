package com.heyu.safetybelt.operator
...
class BleService : Service(), TextToSpeech.OnInitListener {
    ...
    private fun logAlarmToCloud(deviceAddress: String, sensorType: Int?) {
        ...
        val currentUser = cn.leancloud.LCUser.getCurrentUser() ?: return
        AlarmEvent().apply {
    ...
    private fun updateWorkSession(status: String? = null, isOnline: Boolean? = null, partStatuses: Array<SensorStatus>? = null) {
        ...
        if (shouldSave) {
            sessionToUpdate.saveInBackground().subscribe(object : io.reactivex.Observer<LCObject> {
                override fun onSubscribe(d: Disposable) { compositeDisposable.add(d) }
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(NOTIFICATION_CHANNEL_ID, "安全带后台服务", NotificationManager.IMPORTANCE_DEFAULT)
